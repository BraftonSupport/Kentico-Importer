    ...
    protected void DownloadPhotoContent(TreeNode article, string photosFeed)
    {
        ...
        for (int i = 0; i < photos.Count(); i++)
        {
            ...
            try
            {
                ...
                DocumentHelper.AddUnsortedAttachment(article, Guid.NewGuid(), postedFile, ImageHelper.AUTOSIZE, ImageHelper.AUTOSIZE, ImageHelper.AUTOSIZE);

                if (bool.Parse(settings["CreateTeaserImage"]) && mappings["teaserImage"] != "")
                {
                    DocumentHelper.AddAttachment(article, mappings["teaserImage"], postedFile, ImageHelper.AUTOSIZE, ImageHelper.AUTOSIZE, ImageHelper.AUTOSIZE);
                }
